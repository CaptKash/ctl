...
export default function AddRenewalSelectCarScreen() {
  ...
  useEffect(() => {
    if (preselectedCarId && cars) {
      ...
      if (car) {
        router.replace({ pathname: "/renewal/form", params: { carId: String(car.id) } });
      }
    }
  ...
  const handleSelectCar = (car: Car) => {
    ...
    router.push({ pathname: "/renewal/form", params: { carId: String(car.id) } });
  };
      ...
      {isLoading ? (
        ...
        <View style={styles.center}>
          ...
          <Text style={[styles.emptySub, { color: C.textSecondary }]}>
            Add a car first before logging a renewal.
          </Text>
          <Pressable
        ...
        >
          {cars.map((car) => (
            <CarCard key={car.id} car={car} onPress={() => handleSelectCar(car)} accentBg="#CCFBF1" accentIcon="#0D9488" />
          ))}
        </ScrollView>
      )}
      <BottomNav />
